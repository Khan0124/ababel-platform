...
if ($delayed_query->num_rows > 0) {
    while ($row = $delayed_query->fetch_assoc()) {
        ...
        // Bill of Lading delay (>10 days from entry_date AND status is not 'issued')
        ...
        // Tashitim delay (>20 days from bill_of_lading_date AND status is not 'done')
        ...
        // Arrival delay (>60 days from entry_date)
        ...
        // Land Transport delay (>4 days from arrival_date)
        ...
        // Delivery delay (>7 days from arrival_date)
        ...
        // Empty Return delay (>7 days from delivery_date)
...
// Function to display field values with formatting
...
// Function to translate position status
                ...
                <?php if($filters["field$i"]): ?>
                    ...
                    <?php if(count($opts) > 0): ?>
                        <select name="value<?= $i ?>" class="form-select mt-2">
                            ...
                            <?php foreach($opts as $opt_key => $opt_val): ?>
                                <?php 
                                    $option_value = is_array($opt_val) ? $opt_key : $opt_val;
                                    $option_label = is_array($opt_val) ? $opt_val : $opt_val;
                                ?>
                                <option value="<?= $option_value ?>" <?= $filters["value$i"] === $option_value ? 'selected' : '' ?>>
                                    <?= $option_label ?>
                                </option>
                            <?php endforeach; ?>
                        </select>
...
<script>
    // Show modal only once per page visit
