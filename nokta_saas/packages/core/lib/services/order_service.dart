...
class OrderService {
  ...
  // Get all orders
  Future<List<Order>> getOrders({OrderStatus? status, int page = 1, int limit = 50}) async {
    try {
      if (kIsWeb) {
  ...
  Future<Order> updateOrderStatus(int orderId, OrderStatus status) async {
    try {
      if (kIsWeb) {
        ...
        throw UnimplementedError('Order status update not implemented for local DB');
      }
    } catch (e) {
  ...
  Future<void> cancelOrder(int orderId, String reason) async {
    try {
      if (kIsWeb) {
        await _apiService.put('/orders/$orderId/cancel', data: {'reason': reason});
      } else {
        throw UnimplementedError('Order cancellation not implemented for local DB');
      }
    } catch (e) {
    ...
    try {
      if (kIsWeb) {
        ...
        return OrderStatistics(
          ...
          completedOrders: orders.where((o) => o.status == OrderStatus.delivered).length,
          totalRevenue: orders.fold<double>(0, (sum, order) => sum + order.total),
        );
      }
    } catch (e) {
...
final ordersProvider = FutureProvider.autoDispose.family<List<Order>, OrdersQuery>((ref, query) {
  final orderService = ref.watch(orderServiceProvider);
  return orderService.getOrders(status: query.status, page: query.page, limit: query.limit);
});
...
final orderStatisticsProvider = FutureProvider.autoDispose<OrderStatistics>((ref) {
  final orderService = ref.watch(orderServiceProvider);
