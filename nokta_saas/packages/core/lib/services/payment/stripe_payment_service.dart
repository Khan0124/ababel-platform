...
class StripePaymentService implements PaymentGateway {
  StripePaymentService() {
    _stripe = Stripe(publishableKey: ENV.stripePublishableKey);
  }
  ...
  Future<PaymentResult> processPayment({
    ...
    try {
      ...
      return PaymentResult.failure(
        error: PaymentError(code: 'payment_failed', message: e.toString()),
      );
    }
  }
  ...
  Future<PaymentResult> _processCardPayment(
    ...
    final result = await _stripe.confirmPayment(
      ...
      data: PaymentMethodData(type: PaymentMethodType.card, card: cardDetails),
    );
